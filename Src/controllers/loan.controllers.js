...
const calculateLoanEMI = (loanAmount, annualInterestRate, loanPeriodInMonths) => {
  const monthlyInterestRate = annualInterestRate / 12 / 100; // Monthly interest rate
  const emi =
    (loanAmount * monthlyInterestRate * Math.pow(1 + monthlyInterestRate, loanPeriodInMonths)) /
...
export const removeGuarantorFromLoan = async (req, res) => {
  ...
  try {
    ...
    if (!loan.guarantors.includes(guarantorId)) {
      return res.status(400).json({ message: "Guarantor not found in this loan" });
    }

    // Remove the guarantor from the loan
    loan.guarantors = loan.guarantors.filter(id => id.toString() !== guarantorId);
    await loan.save();
...
export const getLoanById = async (req, res) => {
  const { id } = req.params;
  try {
    const loan = await Loan.findById(id).populate("user").populate("guarantors");
    if (!loan) {
      return res.status(404).json({ message: "Loan not found" });
    }
    res.json(loan);
